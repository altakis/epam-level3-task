...
const { valid_credentials, testPassword } = require('../../config/testCredentials.json');
...
describe('UC-3 Test Login form with credentials by passing Username & Password', () => {
    ...
    for (const username of valid_credentials) {
        it(`Test Login form by passing Username: ${username}`, async () => {
            ...
            await inventoryPage.checkPageTitle('Swag Labs');
        });
    }
});
